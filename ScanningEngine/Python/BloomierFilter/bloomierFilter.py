...
class BloomierFilter:
    def __init__(self, keysDict, p, hashSalt = 0, useTable = False):
        #hashSalt: hashing nonce
        #keysDict: input data
        #m: int(len(keysDict)*1.5) -size of filter
        #k: number of hash functions
        #q: bit size - bits per bucket in vector
        ...
        self.q = 16
        q = self.q
        self.hasher = BloomierHasher(hashSalt, m, k, q)
        self.byteSize = getByteSize(q)
        
        oamf = OrderAndMatchFinder(hashSalt, keysDict, m, k, q)
        oam =  oamf.find()
