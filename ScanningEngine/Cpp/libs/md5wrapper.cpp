 ...
 */
std::vector<std::string> md5wrapper::getHashFromFiles(std::vector<FILE*> files)
{
	std::vector<std::string> hashDigests;
  	MD5_CTX context;
	...
	*/
		...
		hashDigests.push_back(convToString(digest));
	}
	return hashDigests;
 }	

std::vector<FILE*> md5wrapper::getFileVector(std::vector<std::string> filePaths)
{
	std::vector<FILE*> bytecode;
	for (std::string i : filePaths){
		FILE *file = fopen(i.c_str(), "rb");
		if (file == NULL){
			std::cout << "Failure to open file.";
		}
		bytecode.push_back(file);
	}
	return bytecode;
}
/*
