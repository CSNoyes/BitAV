...
def add_tx(tx, DB):
    ...
    def type_check(tx, txs):
        if 'type' not in tx:
            return True
        finCheck = tx['type'] not in transactions.tx_check
        return finCheck
...
def add_block(block, DB):
    ...
    def block_check(block, DB):
        ...
        if block['diffLength'] != hexSum(DB['diffLength'],
            ...
            return False
        if length >= 0:
            if tools.detSha(db_get(length, DB)) != block['prevHash']:
                return False
        a = copy.deepcopy(block)
        a.pop('nonce')
        if u'target' not in block.keys():
            return False
        half_way = {u'nonce': block['nonce'], u'halfHash': tools.detSha(a)}
        if tools.detSha(half_way) > block['target']:
            return False
